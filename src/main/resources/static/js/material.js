...
app.controller("AppCtrl", function ($http, $scope) {
    ...
    this.del_entry = function del(id) {
        $http.get('/api/material/del?id='+id).then(function (response){
            window.location.reload();
            window.alert('Матеріал було успішно видалено!');
        });
    };
    ...
    this.insert_entry = function add() {
        ...
        $http.get('/api/material/insert?name='+name).then(function (response){
            window.location.reload();
    ...
    this.update_entry = function upd() {
        ...
        $http.get('/api/material/update?id='+thisId+'&name='+name).then(function (response){
            window.location.reload();
            window.alert('Матеріал було успішно змінено!');
        });
    };
