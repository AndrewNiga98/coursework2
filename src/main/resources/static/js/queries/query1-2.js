...
app.controller("AppCtrl", function ($http, $scope) {
    ...
    let departmentId=1;
    let typeId=1;
    $http.get('/api/query1-2?department_id='+departmentId+'&type_id='+typeId).then(function (response){
        ...
        $http.get('/api/specialties').then(function (response){
            let data = response.data;
            data.sort(function (a, b) { return a.id - b.id; });
            let selector = document.getElementById("Specialty");
            $(selector).empty();
            for (let i = 0; i < data.length; i++) {
                let option = document.createElement("option");
                option.text = data[i].name;
    ...
    $scope.update_request = function add() {
        ...
        let typeIndex = document.getElementById("Specialty").selectedIndex;
        typeId= document.getElementById("Specialty").options[typeIndex].value;
